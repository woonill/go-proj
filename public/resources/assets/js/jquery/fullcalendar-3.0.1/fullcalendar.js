		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2 id="showMonth">&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
							...
							button = $( // type="button" so that it doesn't submit a form
								...
								.click(function(ev) {
									...
									if(!button.hasClass('ui-state-active')) {
										if($("#state").val() == "month") {
											$("input[name=state]").val("list");
											$("#state").val("list");
										} else if ($("#state").val() == "list") {
											$("#state").val("month");
										};
									};
				...
				if (isOnlyButtons) {
					groupChildren
						...
						.first().attr('id', 'month').end()
						.last().attr('id', 'list').end()
						.last().append("<input type='hidden' id='status' value='list'></input>").end()
