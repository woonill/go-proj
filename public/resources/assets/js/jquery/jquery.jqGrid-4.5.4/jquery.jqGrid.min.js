...
dataType:"html",success:function(d){if(f.buildSelect!==void 0)(d=f.buildSelect(d))&&a("td:eq(1)",l).append(d);else a("td:eq(1)",l).append(d);f.defaultValue!==void 0&&a("select",j).val(f.defaultValue);a("select",j).attr({name:g.index||g.name,id:"gs_"+g.name});f.attr&&a("select",j).attr(f.attr);a("select",j).css({width:"100%"});a.jgrid.bincall(c,a("select",j)[0],f);b.autosearch===true&&a("select",j).change(function(){e();return false});d=null}},a.jgrid.ajaxOptions,c.p.ajaxSelectOptions||{}));else{var n,
...
id:"gs_"+g.name});var s;if("string"===typeof n){d=n.split(q);for(s=0;s<d.length;s++)n=d[s].split(r),q=document.createElement("option"),q.value=n[0],q.innerHTML=n[1],k.appendChild(q)}else if("object"===typeof n)for(s in n)n.hasOwnProperty(s)&&(q=document.createElement("option"),q.value=s,q.innerHTML=n[s],k.appendChild(q));void 0!==f.defaultValue&&a(k).val(f.defaultValue);f.attr&&a(k).attr(f.attr);a(j).append(l);a.jgrid.bincall(c,k,f);a("td:eq(1)",l).append(k);!0===b.autosearch&&a(k).change(function(){e();
return false})}}break;case "text":r=void 0!==f.defaultValue?f.defaultValue:"";a("td:eq(1)",l).append("<input type='text' style='width:100%;padding:0px;' name='"+(g.index||g.name)+"' id='gs_"+g.name+"' value='"+r+"'/>");a(j).append(l);f.attr&&a("input",j).attr(f.attr);a.jgrid.bincall(c,a("input",j)[0],f);!0===b.autosearch&&(b.searchOnEnter?a("input",j).keypress(function(a){if((a.charCode||a.keyCode||0)===13){e();return false}return this}):a("input",j).keydown(function(a){switch(a.which){case 13:return false;
...
l[0].rp=x);a("td:eq("+(w-2)+")",l[0]).html(void 0===q.label?e.p.colNames[p]:q.label);a("td:eq("+(w-1)+")",l[0]).append(q.elmprefix).append(o).append(q.elmsuffix);"custom"===this.edittype&&a.isFunction(r.custom_value)&&r.custom_value.call(d,a("#"+i,"#"+g),"set",h);a.jgrid.bincall(d,o,r);t[n]=p;n++}});if(0<n&&(p=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(2*f-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+e.p.id+"_id' value='"+
...
d.append(m);a.jgrid.bincall(c,m,i.searchoptions);a(m).addClass("input-elm").bind("change",function(){g.data=i.inputtype==="custom"?i.searchoptions.custom_value.call(c,a(this).children(".customelement:first"),"get"):a(this).val();b.onchange()});d=a("<td></td>");k.append(d);!0===this.p.ruleButtons&&(m=a("<input type='button' value='-' title='Delete rule' class='delete-rule ui-del'/>"),d.append(m),m.bind("click",function(){for(e=0;e<j.rules.length;e++)if(j.rules[e]===g){j.rules.splice(e,1);break}b.reDraw();
...
b.length&&160===b.charCodeAt(0))b="";var u=a.jgrid.createEl.call(d,m[l].edittype,n,b,!0,a.extend({},a.jgrid.ajaxOptions,d.p.ajaxSelectOptions||{}));a(u).addClass("editable");j?a("span:first",this).append(u):a(this).append(u);a.jgrid.bincall(d,u,n);"select"===m[l].edittype&&void 0!==m[l].editoptions&&!0===m[l].editoptions.multiple&&void 0===m[l].editoptions.dataUrl&&a.jgrid.msie&&a(u).width(a(u).width());e++}}),0<e&&(h.id=c,d.p.savedRow.push(h),a(j).attr("editable","1"),a("td:eq("+g+") input",
...
n=b.jgrid.createEl.call(c,i.edittype,j,e,!0,b.extend({},b.jgrid.ajaxOptions,c.p.ajaxSelectOptions||{}));b(h).html("").append(n).attr("tabindex","0");b.jgrid.bincall(c,n,j);window.setTimeout(function(){b(n).focus()},0);b("input, select, textarea",h).bind("keydown",function(a){a.keyCode===27&&(b("input.hasDatepicker",h).length>0?b(".ui-datepicker").is(":hidden")?b(c).jqGrid("restoreCell",d,f):b("input.hasDatepicker",h).datepicker("hide"):b(c).jqGrid("restoreCell",d,f));if(a.keyCode===13){b(c).jqGrid("saveCell",
