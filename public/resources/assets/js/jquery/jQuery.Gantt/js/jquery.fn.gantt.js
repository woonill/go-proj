...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            createContext: function(obj, data) { return; },
			onDateClick: function(obj, data) { return; },
			onDayClick: function(obj, data) { return; },
			onRemainClick: function(obj, data) { return; },
			saveBarId: function(data) { return; },
			onScroll: function(data) { return; },
            scrollToToday: true,
            showSelector: true,
            selectorId: "selector",
            showNavi: true,
        ...
        */
        ...
        var core = {
            ...
            render: function (element) {
                ...
                if (settings.scrollToToday && settings.showNavi) {
				...
				// add date click event for selection
                if(settings.showSelector == true) {
//					core.setSelectorEvent(element, settings.selectorId);
					var selector = core.setSelectorEvent(element, settings.selectorId);
					$dataPanel.append(selector);
                }

				settings.onRender();
            },

			setSelectorEvent: function(element, selectorId) {
				var selector = $('<div id="selector" style="pointer-events:none;position:absolute;display:none;border:1px solid #FF6347;"/>');
				var dpanel = $("#dataPanel");
				var doffset = dpanel.offset();
				var oid = 0;
				for(var i=0; i<dateIdList.length; i++) (function (id) {
					var obj = $("#"+dateIdList[id]);
					var offset = obj.offset();
					var width = obj.width();
					var height = obj.height();
					var d = {
						top: offset.top - doffset.top,
						left: offset.left - doffset.left,
						width: width,
						height: height,
						cellsize: tools.getCellSize(),
						rowcount: rowcount,
						sid: selectorId,
						bwidth: tools.getCellSize(),
						bheight: tools.getCellSize() * (rowcount+2) - 2,
						btop: offset.top - doffset.top,
						bleft: offset.left - doffset.left,
						margin_left: element.scrollNavigation.panelMargin
					};
					settings.createContext(dateIdList[id], d);
					oid = oid + 1;
				})(oid);

				oid = 0;
				for(var i=0; i<dayIdList.length; i++) (function (id) {
					var obj = $("#"+dayIdList[id]);
					var offset = obj.offset();
					var width = obj.width();
					var height = obj.height();
					var d = {
						...
						bwidth: tools.getCellSize(),
						bheight: tools.getCellSize() * (rowcount+2) - 2,
						btop: offset.top - tools.getCellSize() - doffset.top,
						bleft: offset.left - doffset.left,
						margin_left: element.scrollNavigation.panelMargin
					};
					settings.createContext(dayIdList[id], d);
					oid = oid + 1;
				})(oid);
				
				oid = 0;
				for(var i=0; i<remainIdList.length; i++) (function (id) {
					var dateObj = $("#"+dateIdList[0]);
					var dateOffset = dateObj.offset();
					var obj = $("#"+remainIdList[id]);
					var offset = obj.offset();
					var width = obj.width();
					var height = obj.height();
					var d = {
						...
						sid: selectorId,
						bwidth: tools.getCellSize(),
						bheight: tools.getCellSize() * rowcount - 2,
						btop: dateOffset.top + tools.getCellSize()*2 - doffset.top,
						bleft: offset.left - doffset.left,
						margin_left: element.scrollNavigation.panelMargin
					};
					settings.createContext(remainIdList[id], d);
					oid = oid + 1;
				})(oid);
			...
			viewSelector: function (data) {
				$("#"+data.sid).css({
					"width": data.bwidth+"px",
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        entries.push('<span id=rowheader_span'+ i +' class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.name || '') + '</span>');
                        entries.push('</div>');
						rowcount = rowcount + 1;
            ...
            dataPanel: function (element, width) {
                var dataPanel = $('<div id="dataPanel" class="dataPanel" style="width: ' + width + 'px;"/>');
/*
...
*/
                    ...
                    default:
						...
						for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            dayArr.push('<div class="row date ' + day_class + '" '
                                    ...
                                    + ' <div class="fn-label" id="dh-l-' + tools.genId(rday.getTime()) + '" style="cursor:pointer;">' + rday.getDate() + '</div></div>');
                            dowArr.push('<div class="row day ' + day_class + '" '
                                    + ' id="dw-' + tools.genId(rday.getTime()) + '"  repdate="' + rday.getRepDate() + '"> '
                                    + ' <div class="fn-label" id="dw-l-' + tools.genId(rday.getTime()) + '" style="cursor:pointer;">' + settings.dow[getDay] + '</div></div>');
							dateIdList[i] = "dh-l-" + tools.genId(rday.getTime());
							dayIdList[i] = "dw-l-" + tools.genId(rday.getTime());
                        } //for
            ...
            // the entire chart
            createProgressBar: function (count, days, cls, desc, label, dataObj) {
                var cellWidth = tools.getCellSize() + 2;
                var barMarg = tools.getProgressBarMargin() || 0;
				var slice = 0;
				if(dataObj.isslice == 1) {
					slice = 1;
				}
				var barId = "bar" + count;
				if(slice == 0) {
					var idx = remainIdList.length;
					remainIdList[idx] = barId;
				} else if(slice == 1) {
					settings.saveBarId(dataObj, barId);
				}
                var bar = $('<div id="bar' + count + '" class="bar"><div class="fn-label">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
							width: ((cellWidth - 2) * days) - barMarg - 2
                        })
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
									...
									var slice = 0;
									if(day.dataObj.isslice && day.dataObj.isslice == 1) {
										slice = Math.floor(tools.getCellSize()/2);
									}
                                    var cFrom = parseInt(from.attr("offset")) + slice;
                                ...
                                default:
                                    ...
                                    if ($(_bar).attr('class') == 'bar ganttOrange') {
                                    	_bar.css({ 'top': top, 'left': Math.floor(cFrom) -1 });
                                    } else {
                                    	_bar.css({ 'top': top, 'left': Math.floor(cFrom) -14 });
                                    }
